...
ADMIN_UID = "14431438325"
server2 = "BD"
key2 = "mg24"
BYPASS_TOKEN = "your_bypass_token_here"
WHITELIST_ONLY = False
bot_enabled = True
BOT_OWNER_UID = 14431438325  
PLAYER_NAME_CACHE = {}
...
📱 Player will see:
"PICOS BOT wants to join your team"
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF] PICOS BOT[00FF00]___ PICOS____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @PICOS\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
[C][B][00FFFF]━━━━━━━━━━━
[C][B][FFFFFF]Developer     : PICOS BOT 
"""
    # Send main info
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF] PICOS BOT [00FF00]!!
"""
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="PICOS", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="PICOS", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde PICOS BOT ya meku agar tu noob bolra toh tu g a y hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="PICOS", title_id=title_id)
              ...
              # =================== EMOTE DETECTION ONLY ===================
                    ...
                    try:
                        ...
                        if "MĢ24_GÀMER" in WHITELISTED_UIDS:
                            ...
                            try:
                                ...
                                bot_uid = 13777711848
                                message_text = f" Can't accept Your request Talk to PICOS"
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][FF0000]█▓▒░░ WELCOME TO PICOS BOT ░░▒▓█
[00FF00]»»————-　★　————-««
[FFFFFF]⚡ BOT POWERED BY:[FF0000] PICOS
[FFFFFF]🎯 STATUS: [00FF00]ACTIVE 24/7
[FFFFFF]📱 CONTACT: [FFFF00]@picos0
[00FF00]»»————-　★　————-««
...
# GUILD INFORMATION FF
                        ...
                        if inPuTMsG.strip().startswith('/wlremove'):
                            ...
                            # Check owner
                            if str(response.Data.uid) != "14431438325":
...
"""
                            ...
                            if str(response.Data.uid) != "14431438325":  # Replace with your actual UID
...
"""
                        ...
                        if inPuTMsG.strip() == '/wllist':
                            ...
                            # Check if owner
                            if str(response.Data.uid) != "14431438325":  # Your UID
...
👥 Whitelisted UIDs:"""
                            ...
                            for uid in WHITELISTED_UIDS:
                                if uid != "14431438325":  # Skip owner since already shown
                                    whitelist_msg += f"\n• {xMsGFixinG(uid)}"
                                    count += 1
                                    if count >= 20:
...
[b][i][FFC0CB]youtube: PICOS 99[/b]

[b][c][FFC0CB]subcribe: my_channel[FFFFFF]
 
[b][i][FFA500]telegram: @picos0 [/b]

[b][c][FFA500]telegram contact: @picos0[A52A2A]
...
# In TcPChaT function, update /status command:
                        ...
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "hi"):
                            ...
                            header = f"[c][00FF00]Hey {player_name} [00FFFF]Welcome To PICOS's BOT"
